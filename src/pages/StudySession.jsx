...
import { ArrowLeft, ArrowRight, Pause, Play, HelpCircle } from 'lucide-react'
...
function ModeSelector({ onSelect }) {
  return (
    <div className="max-w-lg mx-auto px-4 py-8">
      <h2 className="text-2xl font-extrabold mb-2">Choose study mode</h2>
      <p className="opacity-60 mb-6 text-sm">How do you want to review today?</p>
...
function SessionEnd({ accuracy, cardsReviewed, timeTaken, deckId, onStudyAgain }) {
  ...
  const message = pct >= 90
    ...
    : pct >= 70
    ? "Really solid session! Keep it up!"
    : pct >= 50
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-12 text-center">
      ...
      <div className="card-base p-6 mb-6 space-y-4" style={{ background: 'var(--color-surface)' }}>
        <div className="flex justify-around">
          <div className="text-center">
            <div className="text-3xl font-extrabold" style={{ color: '#ff9f7a' }}>{pct}%</div>
            <div className="text-xs opacity-60 font-semibold mt-1">Accuracy</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-extrabold" style={{ color: '#c4b5fd' }}>{cardsReviewed}</div>
            <div className="text-xs opacity-60 font-semibold mt-1">Cards</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-extrabold" style={{ color: '#86efac' }}>{timeStr}</div>
            <div className="text-xs opacity-60 font-semibold mt-1">Time</div>
          </div>
        </div>
      </div>

      <div className="flex gap-3">
        <button onClick={onStudyAgain} className="btn-primary flex-1 justify-center">
          Study Again 🔁
        </button>
        <Link to={`/deck/${deckId}`} className="btn-secondary flex-1 justify-center text-center">
          Back to Deck
        </Link>
      </div>
    </div>
...
export default function StudySession() {
  ...
  const filteredCards = mode === 'spaced'
    ? cards.filter(c => true) // SM-2 filtering happens in hook
    : cards

  const session = useStudySession(sessionCards || [], id, mode || 'sequential')
  ...
  const handleModeSelect = (selectedMode) => {
    ...
    setSessionCards(cards2)
  }

  const handleGrade = (gradeValue, e) => {
  ...
  // Keyboard shortcuts
  useEffect(() => {
    if (!mode || sessionDone) return
    const handleKey = (e) => {
      if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') return
      switch (e.key) {
        case ' ': e.preventDefault(); session.flip(); break
        case 'ArrowLeft': session.prev(); break
        case 'ArrowRight': session.next(); break
  ...
  if (!session.currentCard) {
    return (
      <div className="max-w-md mx-auto px-4 py-16 text-center">
        ...
        <Link to={`/deck/${id}`} className="btn-primary">Go to deck</Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => navigate(`/deck/${id}`)} className="flex items-center gap-1 text-sm opacity-50 hover:opacity-100 transition-opacity">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShortcutsOpen(true)} className="w-7 h-7 rounded-full hover:bg-black/5 flex items-center justify-center opacity-40 hover:opacity-70">
            <HelpCircle size={14} />
          </button>
        </div>
      </div>
